  ...
  render() {
    return (
      <div className="CreateJackal">
        <input className="CreateJackal-name"
              name="title"
              placeholder="Title"
...
const JackalsListItem = ({ id, name, body, active }) => {
  return (
    <div className={active ? 'JackalsListItem is-active' : 'JackalsListItem'}>
      <h3 className="JackalsListItem-name">{name}</h3>
...
const ActiveJackal = ({ jackal }) => {
  ...
  return (
    <div className="ActiveJackal">
      <input className="ActiveJackal-name"
             name="name"
             value={jackal.name}
...
ReactDOM.render(<JackalList title="JackalList"/>, document.querySelector('.application'));
